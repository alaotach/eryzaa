...
class BlockchainIntegration {
    ...
    async registerNode() {
        ...
        const existingState = this.loadNodeState();
        if (existingState && existingState.nodeId) {
            console.log("⚠️  Node already registered!");
            console.log(`   Node ID: ${existingState.nodeId}`);
            console.log("   Use 'start' command to make it available for rental");
            return true;
        }

        // Check if this provider already has nodes registered
        try {
            const providerNodes = await this.marketplace.getProviderNodes(this.wallet.address);
            if (providerNodes.length > 0) {
                console.log("⚠️  You already have registered nodes:");
                for (const nodeId of providerNodes) {
                    const nodeInfo = await this.marketplace.getNodeInfo(nodeId);
                    console.log(`   Node ${nodeId}: ${nodeInfo.nodeType}, ${nodeInfo.available ? 'Available' : 'Busy'}`);
                }
                console.log("\n❓ Do you want to register another node? (This will create duplicates)");
                console.log("   Consider using existing node ID instead");
                
                // For now, let's prevent duplicate registrations
                // Save the latest node as the active one
                ...
                const nodeState = {
                    ...
                    pricePerHour: "500000000000000000" // 0.5 AVAX
                };
                this.saveNodeState(nodeState);
                console.log(`✅ Using existing node ID: ${latestNodeId}`);
                return true;
            }
        } catch (error) {
            console.log("Could not check existing nodes, proceeding with registration...");
        }
    ...
    async cleanupDuplicateNodes() {
        ...
        try {
            ...
            console.log(`⚠️  Found ${providerNodes.length} nodes registered to this provider:`);
            ...
            // Keep the latest node, mark others as unavailable
            const latestNodeId = providerNodes[providerNodes.length - 1].toString();
            console.log(`\n🎯 Using latest node ID: ${latestNodeId} as primary`);
            
            // Update node state to use the latest one
            const nodeState = {
                nodeId: latestNodeId,
                address: this.wallet.address,
                registeredAt: new Date().toISOString(),
            ...
            // Make older nodes unavailable
            for (let i = 0; i < providerNodes.length - 1; i++) {
                const nodeId = providerNodes[i];
                try {
                    console.log(`🔄 Making node ${nodeId} unavailable...`);
                    const tx = await this.marketplace.updateNodeAvailability(nodeId, false);
                    await tx.wait();
                    console.log(`   ✅ Node ${nodeId} marked as unavailable`);
                } catch (error) {
                    console.log(`   ⚠️  Could not update node ${nodeId}: ${error.message}`);
                }
            }

            console.log(`\n✅ Cleanup complete. Active node: ${latestNodeId}`);
            return true;
...
async function main() {
    ...
    switch (command) {
        ...
        case 'cleanup':
            await blockchain.cleanupDuplicateNodes();
            break;
        ...
        default:
            ...
            console.log("  cleanup   🧹 Remove duplicate node registrations");
