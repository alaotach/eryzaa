...
build_target() {
    ...
    if command_exists cross && [[ "$target" != "$(rustc -vV | grep host | cut -d' ' -f2)" ]]; then
        cross build --release --target "$target" || error "Failed to build $app_name for $target"
    else
        cargo build --release --target "$target" || error "Failed to build $app_name for $target"
    fi
...
package_builds() {
    ...
    if [ -f "gui-client/target/$target/release/eryzaa-client$ext" ]; then
        cp "gui-client/target/$target/release/eryzaa-client$ext" "$output_dir/"
        log "Copied GUI client for $target"
    fi
    
    if [ -f "gui-rental/target/$target/release/eryzaa-rental$ext" ]; then
        cp "gui-rental/target/$target/release/eryzaa-rental$ext" "$output_dir/"
        log "Copied GUI rental server for $target"
    fi
    
    # Copy CLI client
    if [ -f "client/target/$target/release/client$ext" ]; then
        cp "client/target/$target/release/client$ext" "$output_dir/eryzaa-cli$ext"
...
main() {
    ...
    for target in "${targets[@]}"; do
        ...
        build_target "$target" "gui-client" "GUI Client"
        build_target "$target" "gui-rental" "GUI Rental Server"
